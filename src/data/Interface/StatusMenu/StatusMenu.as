package
{
   ...
   public class StatusMenu extends IMenu
   {
      ...
      {
         super();
         addFrameScript(8, this.frame9, 9, this.frame10, 15, this.frame16);
      ...
      private function OnCategoryChange():void
      {
         ...
         switch (this.CategoryList_mc.selectedEntry.uCategory)
         {
            case STAT_CATEGORY_EFFECTS:
               if (this.StatusData.aEffectGroups.length > 0)
               {
                  ...
                  addEffectGroup = function(param1:Object):void
                  {
                     ...
                     if (param1.sName != SPECIAL_ATTRIBUTES_DISPLAY)
                     {
            ...
            case STAT_CATEGORY_CHARACTER:
               ...
               // BEGIN S.P.E.C.I.A.L. Display
               this.AddSpecialAttributes(this.StatusData.aEffectGroups)
               // END S.P.E.C.I.A.L. Display
      ...
      {
         this.OnCloseMenuFadeOutComplete();
         stop();
      }
      ...
      // BEGIN S.P.E.C.I.A.L. Display
      internal function AddSpecialAttributes(param1:Object) : void
      {
         ...
         if (param1.length > 0)
         {
            ...
            {
               if (effectGroup.sName.length > 0 && effectGroup.sName == SPECIAL_ATTRIBUTES_DISPLAY)
               {
                  _loc1_ = effectGroup;
                  break;
               }
            ...
            // data:XML = describeType(this.StatusData);
            /* for each (node in this.StatusData)
            {
               cNode: String = String(node[0]);
               if (cNode == undefined)
               {
                  continue;
               }
               this.RightScrollList_mc.AddHeader(String(node));
            } */

            if (_loc1_) 
            {
               this.RightScrollList_mc.AddHeader("ATTRIBUTES");
               this.RightScrollList_mc.AddDivider();
               this.RightScrollList_mc.AddSpecialAttributes(_loc1_);
               this.RightScrollList_mc.AddDivider();
            }
         }
      }
